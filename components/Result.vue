<template>
    <card class="sticky top-0 z-50 text-center bg-blue-light text-white card"
        ...
        <div class="flex">
            <div class="px-2 box bg-blue-light">
                <h2 class="text-2xl md:text-4xl font-bold flex items-end leading-none">
                    <ClientOnly>
                        <VueCountUp :end-val="parseFloat(calculatorStore.alcoholMass.toFixed(1))"
                            :options="bakOptions" />
            ...
            <div class="px-2 bg-blue box transition duration-150"
...
// --- Refs for Animation Start Values ---
const bakStartVal = ref(0)
const promilleStartVal = ref(0)
const timeUntilSoberStartVal = ref(0)

// --- Reactive Options for Counters ---
const promilleOptions = computed(() => ({
    startVal: promilleStartVal.value,
    decimalPlaces: 2,
}))

const timeUntilSoberOptions = computed(() => ({
    startVal: timeUntilSoberStartVal.value,
    decimalPlaces: 1,
}))

const bakOptions = computed(() => ({
    startVal: bakStartVal.value,
    decimalPlaces: 1,
}))
...
const reductionFactor = computed(() => {
    const { isMale } = calculatorStore.input
    return isMale ? 0.68 : 0.55
})
...
// --- Watchers to Update Start Values ---
watch(() => calculatorStore.alcoholMass, (newValue, oldValue) => {
    bakStartVal.value = oldValue || 0
})

watch(promille, (newValue, oldValue) => {
    promilleStartVal.value = parseFloat(oldValue) || 0
})

watch(timeUntilSober, (newValue, oldValue) => {
    timeUntilSoberStartVal.value = parseFloat(oldValue) || 0
})
</script>

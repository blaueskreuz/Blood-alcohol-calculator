    ...
    <div class="drink-selection md:mx-16">
        <div class="flex justify-between items-center mb-6 overflow-x-auto scrolling-touch">
            <ul class="flex list-reset -mx-2">
                <li v-for="(drink, index) in calculatorStore.input.drinks" :key="drink.name" class="px-2"
                    :class="{ 'ml-4 sm:ml-8 md:ml-0': index === 0 }" @click="select(drink, index)">
                    <span
                        class="transition duration-150 px-6 py-2 text-white rounded font-bold cursor-pointer text-lg inline-block"
                        :class="[active === drink.name ? 'bg-blue' : 'bg-grey-400']">{{ $t(drink.name) }}</span>
                </li>
                <li class="px-2">
                    <button
                        class="transition duration-150 px-6 py-2 rounded font-bold cursor-pointer text-lg inline-block bg-red-400"
